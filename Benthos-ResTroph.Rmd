...
```{r map}
 f%>% addCircles(popup = ~paste0("station :",STi$st,"<br/>",
   STi$sp, "<br/>","effectifs : ",STi$eff,"/m2"),fillOpacity = 1,
   opacity =.8,
   radius = STi$eff/15, color = ifelse(
        test =STi$camp == "1987",
        yes = "#FFA500",
        no = ifelse(
        test =STi$camp == "2001",
...
fi%>% addCircles(popup = ~paste0("station :",mambiI$st,"<br/>",
               "année :",mambiI$camp,"<br/>",
  "indice: ",mambiI$Vind),fillOpacity = 0,opacity =1,
      radius = 10, color =ifelse(
        test =mambiI$Tind == "AMBI",
        yes = pal2(mambiI$Vind),
        no = pal(mambiI$Vind)))
