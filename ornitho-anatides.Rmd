---
title: "Comptages ornitho - Anatidés"
output: 
...
# anatidees
spc<-c("Bernache cravant","Canard siffleur",
      "Canard chipeau","Canard pilet",
      "Canard souchet","Sarcelle d'hiver","Canard colvert","Tadorne de Belon",
      "Macreuse noire")
dataMa<-dataT[dataT$TAXO_VERNACUL %in% c(spc),]
dataMa<-dataMa[dataMa$annee >1999,]
dataMa<-aggregate(SommeDeOBSE_NOMBRE~TAXO_VERNACUL+
                   annee+mois,data=dataMa,mean)

gradCol <- colorRampPalette(c("#255668","#106673","#00767A","#00877F", "#17A77E","#47B679","#6FC471","#97D267","#C0E05E"), space = "rgb")(9)
```
...
<font size="4">
<a href='https://www.reservebaiedesaintbrieuc.com/sbaa_reservebaie/RSerena/ornitho-anatides.html'>`Anatidés`</a></font>
...
Résultats des moyennes mensuelles des comptages pour le mois de 
**`r month(ddc[2],label = TRUE ,abbr = FALSE)`** 
```{r}
nmois<-ddc[2]
gac(nmois)
...
### `r nommois`
Résultats des moyennes mensuelles des comptages pour le mois de **`r nommois`** 
```{r}
gac(nmois)
nmois<-ddc[2]+7
if(nmois>12){nmois=nmois-12}
...
### `r nommois`
Résultats des moyennes mensuelles des comptages pour le mois 
**`r nommois`** 
```{r}
gac(nmois)
nmois<-ddc[2]+10
